function [good, Strain_p,Strain_e,Strain_a,Tau_m,Tau_a,G_right,G_left,G,K,nn] = HystLoop_gai(eStrain,tau,N_HystLoop, fnum)
    eSpMax_Index=[];
    eSpMin_Index=[];
    a1=[];
    b1=[];
...
%     N_HystLoop=100;
    for k=1:N_HystLoop %计算周期数
       ...
       if(eSpMax_Index(k)<280)

        x=fStrain_temp(eSpMax_Index(k):(eSpMax_Index(k)+40));
        y=ftau_temp(eSpMax_Index(k):(eSpMax_Index(k)+40));
        size(x);
        size(y);
        p=polyfit(x,y,1);
        ...
        eSp_Plastic(k)=-b1(k)/a1(k);
             ...
             x1=fStrain_temp(eSpMax_Index(k):320);
            x2=fStrain_temp(1:(40-(320-eSpMax_Index(k))));
            x=[x1',x2'];
    ...
    %左侧G
       ...
       if(eSpMin_Index(k)<280)

        x=fStrain_temp(eSpMin_Index(k):(eSpMin_Index(k)+40));
        y=ftau_temp(eSpMin_Index(k):(eSpMin_Index(k)+40));
        p=polyfit(x,y,1);
        ...
        eSp_Plastic(k)=-b1(k)/a1(k);
             ...
             x1=fStrain_temp(eSpMin_Index(k):320);
            x2=fStrain_temp(1:(40-(320-eSpMin_Index(k))));
            x=[x1',x2'];
                    y1=ftau_temp(eSpMin_Index(k):320);
            y2=ftau_temp(1:(40-(320-eSpMin_Index(k))));
            y=[y1',y2'];
